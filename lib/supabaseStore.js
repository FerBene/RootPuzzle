...
// Los datos legacy pueden contener una precisión que no coincide con las
// partes disponibles. La restricción de Postgres exige que ambos conceptos
// sean coherentes, por lo que la precisión se deriva siempre de las partes.
function normalizedDateParts(yearValue, monthValue, dayValue) {
  const year = integerInRange(yearValue, 1, 9999);
  if (year === null) return { year: null, month: null, day: null, precision: null };

  const month = integerInRange(monthValue, 1, 12);
  if (month === null) return { year, month: null, day: null, precision: 'year' };

  const day = integerInRange(dayValue, 1, daysInMonth(year, month));
  if (day === null) return { year, month, day: null, precision: 'month' };

  return { year, month, day, precision: 'day' };
}

function normalizedCertainty(value) {
  return value === 'approx' ? 'approx' : 'exact';
}

function normalizedPlacePrecision(value) {
  return ['country', 'region', 'city', 'locality'].includes(value) ? value : null;
}

// --- Mapeadores de Entidades (JS camelCase <-> Postgres snake_case) ---

function personToDb(p, treeId) {
  const birth = normalizedDateParts(p.birthYear, p.birthMonth, p.birthDay);
  const death = normalizedDateParts(p.deathYear, p.deathMonth, p.deathDay);
  ...
  return {
    ...
    nickname: p.nickname || '',
    email: p.email || '',
    profile_image: p.profileImagePath || p.profileImage || '',
    sex: p.sex || '',
    ...
    birth_date_certainty: normalizedCertainty(p.birthDateCertainty),
    birth_place_id: ensureUuid(p.birthPlaceId) || null,
    birth_place_precision: normalizedPlacePrecision(p.birthPlacePrecision),
    birth_place_certainty: normalizedCertainty(p.birthPlaceCertainty),
    death_date: p.deathDate || '',
    death_year: death.year,
    death_month: death.month,
    death_day: death.day,
    death_date_precision: death.precision,
    death_date_certainty: normalizedCertainty(p.deathDateCertainty),
    death_place: p.deathPlace || '',
    death_place_id: ensureUuid(p.deathPlaceId) || null,
    death_place_precision: normalizedPlacePrecision(p.deathPlacePrecision),
    death_place_certainty: normalizedCertainty(p.deathPlaceCertainty),
...
export async function listAccessibleTrees() {
  ...
  // Use only columns present in both the legacy and current schemas. Optional
  // soft-delete/public metadata must not prevent loading the remote tree.
...
export async function getRemoteTree(remoteTreeId) {
  ...
  try {
    ...
    const people = await Promise.all((peopleRows || []).map(async (row) => {
      const person = dbToPerson(row);
      if (!person.profileImagePath || person.profileImagePath.startsWith('data:') || person.profileImagePath.startsWith('http')) return person;
      const { data: signed, error: signedError } = await supabase.storage
        .from('people-images')
        .createSignedUrl(person.profileImagePath, 60 * 60);
      return { ...person, profileImage: signedError ? person.profileImagePath : signed?.signedUrl || person.profileImagePath };
    }));
...
export async function uploadPersonImage({ treeId, personId, file }) {
  if (!supabase) return { data: null, error: new Error('Supabase no configurado') };
  if (!treeId || !personId || !file) return { data: null, error: new Error('Faltan datos para subir la imagen') };
  const path = `${treeId}/${personId}/${crypto.randomUUID()}.webp`;
  const { error } = await supabase.storage.from('people-images').upload(path, file, {
    cacheControl: '31536000',
    contentType: 'image/webp',
    upsert: false
  });
  if (error) return { data: null, error };
  const { data: signed, error: signedError } = await supabase.storage.from('people-images').createSignedUrl(path, 60 * 60);
  if (signedError) return { data: { path, url: '' }, error: null };
  return { data: { path, url: signed?.signedUrl || '' }, error: null };
}

export async function removePersonImage(path) {
  if (!supabase || !path || path.startsWith('data:') || path.startsWith('http')) return { data: null, error: null };
  return supabase.storage.from('people-images').remove([path]);
}
  ...
  try {
    ...
    if (treeId) {
      const { data: updated, error: updateErr } = await supabase
        ...
        .select('id')
        .single();
      // Guardar el nombre del árbol y guardar sus entidades son operaciones
      // independientes. Un editor puede tener permiso para modificar people
      // pero no para actualizar la metadata de trees.
      if (updateErr && updateErr.code !== 'PGRST116') {
        console.warn('No se pudo actualizar la metadata del árbol; se continuará con las entidades.', updateErr);
      }
    }
    ...
    if (placesDb.length > 0) {
      ...
      peopleDb = peopleDb.map((person) => ({ ...person, birth_place_id: canonicalPlaceId(person.birth_place_id), death_place_id: canonicalPlaceId(person.death_place_id) }));
    }
    if (peopleDb.length > 0) {
      const { error: pErr } = await supabase.from('people').upsert(peopleDb, { onConflict: 'id' });
      if (pErr) {
        // El lote es atómico: una ficha legacy inválida impide guardar también
        // las fichas correctas. El fallback individual aísla la fila dañada.
        console.warn('Falló el upsert masivo de personas; se reintentará por ficha.', pErr);
        const failedPeople = [];
        for (const person of peopleDb) {
          const { error: personErr } = await supabase.from('people').upsert(person, { onConflict: 'id' });
          if (personErr) failedPeople.push({ id: person.id, error: personErr });
        }
        if (failedPeople.length > 0) {
          const firstFailure = failedPeople[0];
          const isolatedError = new Error(`No se pudieron guardar ${failedPeople.length} ficha(s). Primera ficha: ${firstFailure.id}. ${firstFailure.error.message}`);
          isolatedError.code = firstFailure.error.code;
          isolatedError.details = failedPeople;
          return { data: null, error: isolatedError };
        }
      }
    }
