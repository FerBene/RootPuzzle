...
export async function getRemoteTree(remoteTreeId) {
  ...
  try {
    // 1. Obtener la metadata del árbol
    let treeRecord = null;
    if (remoteTreeId && uuidRegex.test(remoteTreeId)) {
      const { data, error } = await supabase
        .from(TREES_TABLE)
        .select('id, name, description, root_person_id')
        .eq('id', remoteTreeId)
        .maybeSingle();
      if (!error && data) treeRecord = data;
    }
...
export async function saveRemoteTree({ id, data }) {
  ...
  try {
    ...
    if (!treeId) {
      ...
      if (existingTree?.[0]?.id) {
        treeId = existingTree[0].id;
        await supabase
          .from(TREES_TABLE)
          .update({ name: treeName })
          .eq('id', treeId);
    ...
    const peopleDb = (data?.people || []).map((p) => personToDb(p, treeId));
